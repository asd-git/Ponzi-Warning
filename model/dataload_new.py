...
class GraphData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return [[torch.from_numpy(self.features_onehot[index][0]).float(),torch.from_numpy(self.features_onehot[index][1]).float()],  # node_features
                torch.from_numpy(self.adj_list[index]).float(),  # adjacency matrix
                int(self.labels[index])]
...
class DataReader():
    ...
    def __init__(self,
        ...
        data['adj_list'] = self.read_graph_adj(list(filter(lambda f: f.find('_A.txt') >= 0, files))[0], nodes, graphs)
        ...
        if self.use_cont_node_attr:
            data['attr'] = self.read_node_features(list(filter(lambda f: f.find(f'_{self.dataset}_{self.trans_normal_type}_{self.trans_Normal}_node_attributes') >= 0, files))[0],
                                                   nodes, graphs,
    ...
    def choice_edge_num_spilit_idx(self,i,edges_num, subG_dataset):
        ...
        ######################################################################
        # 划分好的 训练集、验证集、测试集
        split_idx_path = f'/public/zjj/jj/data/{subG_dataset}/Dataset_tvt_idx/{self.rnd_states[i]}/'
    ...
    def choice_edge_num_spilit_idx_new(self, i, edges_num, proportion):
        ...
        if proportion == '1:n':
            # Todo 特征变了这里就要改
            save_path = f'./data/{self.dataset}/graph_class_data/{self.rnd_states[i]}/'
    ...
    def read_node_features(self, fpath, nodes, graphs, fn, ftype):
        if ftype == 'f':
            ...
            node_features_lst = np.vstack(pkl.load(open(os.path.join(self.data_dir, fpath), 'rb')))
            node_features_lst = [node_features_lst[graphs[graph]] for graph in graphs]
